...
class RuleEventRepositoryTest < ActiveSupport::TestCase
  ...
  test '.dump' do
    ...
    travel_to("2022-02-26 22:22:22 UTC") do
      event = RuleEvent.new(shopify_identifier: 'xyz', hooklys_identifier: '123')
      event.add_detail(:error, "test 2")
      @repository.add_event(event)
    end

    expected = [
      { "shopify_identifier" => "abc", "hooklys_identifier" => "def", "details" => [{ "timestamp"=>"2022-02-26 11:11:11", "level"=>"info", "message"=>"test 1" }]},
      { "shopify_identifier" => "xyz", "hooklys_identifier" => "123", "details" => [{ "timestamp"=>"2022-02-26 22:22:22", "level"=>"error", "message"=>"test 2" }]}
    ]
  ...
  test '.load' do
    ...
    assert_equal("abc", first_event.shopify_identifier)
    assert_equal("def", first_event.hooklys_identifier)
    ...
    assert_equal("xyz", second_event.shopify_identifier)
    assert_equal("123", second_event.hooklys_identifier)
    ...
    assert_equal("", third_event.shopify_identifier)
    assert_equal("", third_event.hooklys_identifier)
  ...
  test "#add_event kept a maximum of the last 10" do
    ...
    extra_event = RuleEvent.new(shopify_identifier: "xyz", hooklys_identifier: "123")
